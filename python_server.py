...
# Database path - can be configured via DB_PATH in .env (relative to BASE_DIR or absolute path)
_db_path_str = os.getenv("DB_PATH", "app.db")
DB_PATH = Path(_db_path_str) if Path(_db_path_str).is_absolute() else BASE_DIR / _db_path_str
...
print("SMTP CONFIG:", SMTP_HOST, SMTP_PORT, SMTP_USER, SMTP_FROM, flush=True)
...
class SessionMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        ...
        user_id = request.session.get("user_id") or request.session.get("pending_mfa_email")

        if user_id:
            now = time.time()
            last_activity = request.session.get("last_activity", now)

            # ⏱ set timeout (e.g. 15 mins)
...
def is_valid_email(email: str, role: str) -> bool:
    email = email.strip().lower()

    if role == "student":
        pattern = r"^[a-z0-9._%+-]+@phinmaed\.com$"
    elif role in ("professor", "administrator"):
...
def is_valid_name(name: str) -> bool:
    ...
    # Require at least 3 words (Juan Dela Cruz)
    parts = name.split()
    ...
    for part in parts:
        if not re.match(r"^[A-Z][a-zA-Z'-]{1,30}$", part):
            return False
...
def send_signin_alert_email(to_email: str) -> bool:
    ...
    try:
        ...
        print("SIGN-IN ALERT EMAIL FAILED:", str(exc), flush=True)
        return False
...
def signin(payload: SignInPayload, request: Request):
    request.session.clear()  # 🔥 MOVE THIS TO TOP
    ...
    with get_conn() as conn:
        ...
        # MFA FLOW
        print("CURRENT MFA EMAIL:", request.session.get("pending_mfa_email"))
        if AUTH_MFA_ENABLED:
...
def signup(payload: SignUpPayload, request: Request):
    ...
    if not is_valid_name(full_name):
        raise HTTPException(
          status_code=400,
          detail="Full name must be in format like 'Juan Dela Cruz' (at least 3 words, capitalized)."
        )
    ...
    if not is_valid_email(email, role):
       if role == "student" and not email.endswith("@phinmaed.com"):
         raise HTTPException(
        status_code=400,
        detail="Students must use a phinmaed.com email."
    )

    if role in ("professor", "administrator") and not email.endswith("@gmail.com"):
     raise HTTPException(
        status_code=400,
        detail="Professors and administrators must use a gmail.com email."
    )
...
def resend_mfa(request: Request):
    ...
    with get_conn() as conn:
        ...
        conn.execute(
            "UPDATE login_mfa_codes SET used = 1 WHERE user_email = ? AND used = 0",
            (email,),
        )
        ...
        print("RESEND MFA TO:", email, flush=True)
...
def verify_mfa(payload: SignInMfaPayload, request: Request):
    email = request.session.get("pending_mfa_email")
    ...
    with get_conn() as conn:
        record = get_valid_login_mfa_code(conn, email, payload.code)

        if not record:
            raise HTTPException(status_code=400, detail="Invalid or expired code.")

        mark_login_mfa_used(conn, record["id"])
...
def debug_contract_code():
    ...
    code = blockchain_audit.w3.eth.get_code(
        blockchain_audit.w3.to_checksum_address(BLOCKCHAIN_CONTRACT_ADDRESS)
    )
    return {
        "connected": True,
