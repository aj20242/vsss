...
const attendance = []; // replace with DB later
...
await transporter.sendMail({
  from: '"System OTP" <yourgmail@gmail.com>',
  to: email,
  subject: 'OTP Code',
...
function buildNotifications({ userEmail, attendanceSummary, chainValid, tickets, role = '', activities = [] }) {
  ...
  if (!chainValid && role && role.toLowerCase() === 'administrator') {
    items.push({
      id: makeId('audit'),
      level: 'warning',
...
if (!['gmail.com', 'phinmaed.com'].includes(emailDomain)) {
  return res.status(400).json({
    message: 'Only gmail.com or phinmaed.com allowed'
  });
}
...
if (!allowedDomains.includes(emailDomain)) {
  return res.status(400).json({
    message: 'Only gmail.com or phinmaed.com emails allowed'
  });
}
if (!email.includes('@')) {
  return res.status(400).json({ message: 'Invalid email format' });
...
app.post('/api/auth/signin', authLimiter, async (req, res) => {
  try {
    ...
    req.session.cookie.maxAge = 10 * 1000; // 5 minutes
  } catch (err) {
    return res.status(500).json({ message: 'Server error. Please try again.' });
  }
});

app.post('/api/auth/signin/verify-mfa', (req, res) => {
  const { code } = req.body;
...
app.get('/api/attendance/students-unmarked', async (req, res) => {
  try {
    ...
    // filter students who already have attendance
...
app.get('/api/students', async (req, res) => {
  try {
    const rows = await new Promise((resolve, reject) => {
      db.all(
        `SELECT id, full_name, email, role, created_at FROM users WHERE role = 'student' ORDER BY full_name ASC`,
        [],
        (err, rows) => {
          if (err) reject(err);
          else resolve(rows || []);
...
app.delete('/api/admin/tickets/:ticketId', async (req, res) => {
  try {
    ...
    await new Promise((resolve, reject) => {
      db.run('DELETE FROM support_tickets WHERE public_id = ?', [ticketId], (err) => {
        if (err) reject(err);
        else resolve();
      });
    });
